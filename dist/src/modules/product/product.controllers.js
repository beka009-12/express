...
exports.getAllProductsForUsers = exports.updateProduct = exports.deleteProduct = exports.getProductById = exports.getProduct = exports.createProduct = void 0;
...
const createProduct = async (req, res) => {
    try {
        ...
        const { categoryId, brandId, title, description, sizes, colors, price, newPrice, stockCount, tags, } = req.body;
        ...
        if (files && files.length > 0) {
            ...
            for (const file of files) {
                ...
                if (error)
                    throw error;
                uploadedUrls.push(`${process.env.SUPABASE_URL}/storage/v1/object/public/product-image/${data.path}`);
            }
        }
...
const updateProduct = async (req, res) => {
    try {
        const { id } = req.params;
        const { categoryId, brandId, title, description, images, sizes, colors, price, newPrice, stockCount, tags, } = req.body;
        if (!req.user?.id) {
            return res.status(401).json({ message: "Не авторизован" });
        }
        const product = await prisma_1.prisma.product.findUnique({
        ...
        const updatedProduct = await prisma_1.prisma.product.update({
            ...
            data: {
                ...
                stockCount: stockCount !== undefined ? Number(stockCount) : product.stockCount,
...
const getAllProductsForUsers = async (req, res) => {
    try {
        ...
        if (brand)
            filters.brand = String(brand);
        if (inStock)
            filters.inStock = inStock === "true";
        if (minPrice || maxPrice) {
            filters.price = {};
            if (minPrice)
                filters.price = Number(minPrice);
            if (maxPrice)
                filters.price = Number(maxPrice);
        }
        const products = await prisma_1.prisma.product.findMany({
            where: {
                ...filters,
...
exports.getAllProductsForUsers = getAllProductsForUsers;
